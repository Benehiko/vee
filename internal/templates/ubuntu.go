...
// NewUbuntuServerConfigNoISO returns a VMConfig for an Ubuntu Server VM without
// downloading or attaching the install ISO. Used when --no-auto-install is set
// and the target disk already has an OS.
func NewUbuntuServerConfigNoISO(p provider.Provider, version images.UbuntuVersion, name string) (*vm.VMConfig, error) {
	if name == "" {
		name = utils.GeneratePetname()
	}
