...
- Omarchy publishes x86_64 ISOs only. On x86_64 hosts the guest runs hardware-accelerated (KVM); on Apple Silicon the template pins `arch: x86_64` in `vm.yaml` and vee runs the guest under TCG emulation with the system `qemu-system-x86_64` (e.g. `brew install qemu`). The unattended install reads from the ISO's offline package mirror, so even emulated it lands in tens of minutes (~10 on an M4 Max); the desktop works but renders in software.
