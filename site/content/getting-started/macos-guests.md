...
5. **Authorize Screen Sharing.** macOS creates the privacy database those permissions live in during the guest's first boot, and vee can only write to it while the guest is stopped — so `vee create` waits for provisioning to finish inside the guest (up to six minutes; usually well under one), restarts it, and waits for it to come back before reporting anything. It happens once per guest. With `--no-start` there is nothing to cycle, so the grants land on the second `vee start` instead.
