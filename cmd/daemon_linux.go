...
const (
	daemonInstallShort = "Install and enable the vee systemd system service"
	daemonInstallLong  = `Install /etc/systemd/system/vee.service as a system-level unit
...
Requires root (sudo).`
	daemonUninstallShort = "Disable and remove the vee systemd system service"
)

// daemonInstall is the Linux service installer: systemd system unit plus
// the vfio modprobe, polkit, and udev rules vee needs on a Linux host.
func daemonInstall(ctx context.Context) error {
	...
	if err := uninstallLegacyUserUnit(ctx); err != nil {
...
func daemonUninstall(ctx context.Context) error {
	return uninstallSystemdUnit(ctx)
}
