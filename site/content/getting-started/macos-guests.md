...
- **Screen Sharing usually needs one manual step.** Provisioning enables the service and runs ARD's `kickstart`, so the guest listens on 5900 — but on macOS 26 kickstart can no longer grant screen recording or control on its own, and says so: *"must be enabled from System Settings or via MDM"*. A session is then refused with *"Screen Sharing is not permitted on <host>"*. The fix is to toggle Screen Sharing in the guest (System Settings > General > Sharing), which needs one visit to its screen, or to manage the guest with MDM. `vee view` reports separately when nothing is listening at all. **SSH is the dependable route and needs no manual step.**
