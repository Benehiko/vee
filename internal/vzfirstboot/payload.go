...
#    screen-sharing agent behind TCC grants this script cannot create; vee
#    writes those into the guest's disk from the host instead, at the first
#    start that finds the privacy database macOS creates on this boot.
...
# enabled from System Settings or via MDM") — so a session is refused with
# "Screen Sharing is not permitted on <host>" until vee writes those grants
# from the host. It is still run because it configures everything it can and
# helps on older guests; failures are non-fatal.
