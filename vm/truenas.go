...
func EnsureTrueNASAPIKey(cfg *VMConfig, ip, storagePath string, promptFn func(prompt string) (string, error)) (apiKey, adminUser string, err error) {
	...
	username, err := promptFn("TrueNAS admin username: ")
	if err != nil {
		return "", "", fmt.Errorf("read username: %w", err)
	}
	username = strings.TrimSpace(username)

	password, err := promptFn("TrueNAS admin password: ")
	if err != nil {
	...
	cfg.TrueNASAPIKey = key
	cfg.TrueNASUser = username
	if err := SaveConfig(storagePath, cfg); err != nil {
		return "", "", fmt.Errorf("save vm.yaml: %w", err)
	}
